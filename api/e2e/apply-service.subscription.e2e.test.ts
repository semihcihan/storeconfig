...
import { findSubscriptionId } from "../src/domains/subscriptions/pricing-service";
import { z } from "zod";
...
describe("Apply Service Subscription Integration Tests", () => {
  ...
  describe("Subscription Update Tests", () => {
    ...
    describe("Subscription Pricing Updates", () => {
      it("should add pricing to subscription that had no pricing", async () => {
              ...
              subscriptions: [
                {
                  ...
                  pricing: {
                    ...
                    prices: [
                      ...
                      { price: "4.99", territory: "GBR" }, // considered equalized pricing with USA 4.99
                      { price: "1.99", territory: "DEU" },
                    ],
                  },
                },
              ],
        ...
        expect(updatedSubscription?.pricing).toBeDefined();
        expect(updatedSubscription?.pricing?.prices).toHaveLength(2);
        expect(updatedSubscription?.pricing?.prices).not.toContainEqual({
          price: "4.99",
          territory: "GBR",
        });
        expect(updatedSubscription?.pricing?.prices).toContainEqual({
          price: "4.99",
          territory: "USA",
        });
        expect(updatedSubscription?.pricing?.prices).toContainEqual({
      ...
      it("should update existing prices for subscription", async () => {
        ...
        expect(updatedSubscription?.pricing?.baseTerritory).toBe("USA");
        expect(updatedSubscription?.pricing?.prices).toContainEqual({
          price: "1.99",
          territory: "USA",
        });
      ...
      it("should verify territories can be removed from subscription pricing", async () => {
        const uniqueId = generateTestIdentifier();
        ...
        expect(updatedSubscription?.pricing?.prices).toHaveLength(1);
        expect(updatedSubscription?.pricing?.prices).toContainEqual({
          price: "4.99",
          territory: "USA",
        });
      ...
      it("should change base territory in subscription pricing", async () => {
              ...
              subscriptions: [
                {
                  ...
                  pricing: {
                    baseTerritory: "GBR",
                    prices: [{ price: "2.99", territory: "GBR" }],
                  },
                },
              ],
            },
        ...
        expect(updatedSubscription?.pricing?.prices).toHaveLength(1);
        // can't check the item because it can be both USA or GBR
  ...
  describe("Subscription Complex Update Combinations", () => {
    ...
    it("should create subscription with pricing for USA/GBR but availability only for USA (pricing territories as subset of availability)", async () => {
      ...
      expect(createdSubscription?.pricing?.prices).toHaveLength(2);
      ...
      expect(createdSubscription?.pricing?.prices).toContainEqual({
        price: "4.99",
        territory: "USA",
      });
      expect(createdSubscription?.pricing?.prices).toContainEqual({
  ...
  describe("Subscription Error Handling Tests", () => {
              ...
              subscriptions: [
                {
                  ...
                  availability: {
                    availableInNewTerritories: false,
                    availableTerritories: ["USA", "GBR", "TUR"],
                  },
                },
              ],
            },
    ...
    describe("Validation Edge Cases", () => {
      ...
      it("should handle subscription with empty prices array", async () => {
        ...
        expect(createdGroup?.subscriptions?.[0]?.pricing).toBeUndefined();
  ...
  describe("Subscription Pricing Tests", () => {
    it("should create subscription with single price and discover additional prices via API", async () => {
      const uniqueId = generateTestIdentifier();
      ...
      const subscription = createdGroup?.subscriptions?.[0];
      expect(subscription?.pricing?.prices).toHaveLength(1);
      expect(subscription?.pricing?.baseTerritory).toBe("USA");
      expect(subscription?.pricing?.prices?.[0]?.territory).toBe("USA");
      expect(subscription?.pricing?.prices?.[0]?.price).toBe("9.99");

      const rawResponse = await fetchSubscriptionGroups(TEST_APP_ID);
      ...
      const pricesResponse = await fetchSubscriptionPrices(subscriptionId);

      // Verify that the API returns pricing data
      expect(pricesResponse).toBeDefined();
      expect(pricesResponse.data).toBeDefined();
      expect(Array.isArray(pricesResponse.data)).toBe(true);
      expect(
        pricesResponse.data.map((p) => p.relationships?.territory?.data?.id)
      ).toContain("USA");
      expect(
        pricesResponse.data.map((p) => p.relationships?.territory?.data?.id)
      ).toContain("GBR");
      ...
      // The API might return more pricing information than what we initially set
      // This demonstrates how the API can provide additional pricing details
      logger.info(
        "   ✅ Created subscription with single price and fetched additional pricing"
      );
    });
  });
