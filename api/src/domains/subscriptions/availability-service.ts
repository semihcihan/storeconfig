...
import { AppStoreModelSchema, AvailabilitySchema } from "@semihcihan/shared";
import { z } from "zod";
import {
...
export async function updateSubscriptionAvailability(
  ...
  const existingAvailability =
    await getSubscriptionAvailability(subscriptionId);

  // TODO: Note we are calling the same API for both create and update. Check if we need to check if availability exists.
  // Maybe we can just call createSubscriptionAvailabilityForSubscription directly without checking if availability exists.
