...
type SubscriptionsResponse = components["schemas"]["SubscriptionsResponse"];
...
export async function fetchSubscriptionGroups(
  ...
  if (response.error) {
    ...
    throw new ContextualError("Failed to fetch subscription groups", {
...
export async function fetchSubscriptionGroupLocalizations(
  groupId: string
  ...
  if (response.error) {
    throw new ContextualError(
      "Failed to fetch subscription group localizations",
      {
        appleError: response.error,
      }
    );
...
export async function fetchSubscriptionsForGroup(
  groupId: string
): Promise<SubscriptionsResponse> {
  const config = API_FIELD_CONFIGS.subscriptionGroups;

  const response = await api.GET("/v1/subscriptionGroups/{id}/subscriptions", {
    params: {
      path: { id: groupId },
      query: {
        limit: API_LIMITS.DEFAULT_LIMIT_v1,
  ...
  if (response.error) {
    throw new ContextualError("Failed to fetch subscriptions for group", {
      appleError: response.error,
    });
  }
...
export async function fetchSubscriptionLocalizations(
      ...
      params: {
        ...
        query: {
          ...
          "fields[subscriptionLocalizations]":
            config.fieldsSubscriptionLocalizations as any,
        },
      },
    }
  );
  ...
  if (response.error) {
    throw new ContextualError("Failed to fetch subscription localizations", {
      appleError: response.error,
    });
  }
...
export async function fetchSubscriptionIntroductoryOffers(
  ...
  if (response.error) {
    throw new ContextualError(
      "Failed to fetch subscription introductory offers",
      {
        appleError: response.error,
      }
    );
...
export async function fetchSubscriptionPromotionalOffers(
  ...
  if (response.error) {
    throw new ContextualError(
      "Failed to fetch subscription promotional offers",
      {
        appleError: response.error,
      }
    );
...
export async function fetchSubscriptionAvailability(
  ...
  if (response.error) {
    ...
    throw new ContextualError("Failed to fetch subscription availability", {
      appleError: response.error,
    });
  }
...
export async function fetchSubscriptionAvailabilityTerritories(
  ...
  if (response.error) {
    throw new ContextualError(
      "Failed to fetch subscription availability territories",
      {
        appleError: response.error,
      }
    );
...
export async function fetchSubscriptionPrices(
  ...
  if (response.error) {
    throw new ContextualError("Failed to fetch subscription prices", {
      appleError: response.error,
    });
  }
...
export async function deleteSubscriptionGroupLocalization(
  ...
  if (response.error) {
    if (isNotFoundError(response.error)) {
      logger.debug(
        `Subscription group localization ${localizationId} was already deleted (404)`
      );
    } else {
      throw new ContextualError("Apple API error", {
        appleError: response.error,
...
export async function deleteSubscriptionLocalization(
  ...
  if (response.error) {
    if (isNotFoundError(response.error)) {
      logger.debug(
        `Subscription localization ${localizationId} was already deleted (404)`
      );
    } else {
      throw new ContextualError("Apple API error", {
        appleError: response.error,
...
export async function getSubscriptionAvailability(
  ...
  if (response.error) {
    ...
    throw new ContextualError("Failed to get subscription availability", {
      appleError: response.error,
    });
  }
...
export async function deleteSubscriptionIntroductoryOffer(
  ...
  if (response.error) {
    if (isNotFoundError(response.error)) {
      logger.debug(
        `Subscription introductory offer ${offerId} was already deleted (404)`
      );
    } else {
      throw new ContextualError("Apple API error", {
        appleError: response.error,
...
export async function deleteSubscription(
  ...
  if (response.error) {
    if (isNotFoundError(response.error)) {
      logger.debug(`Subscription ${subscriptionId} was already deleted (404)`);
    } else {
      throw new ContextualError("Apple API error", {
        appleError: response.error,
      });
...
export async function deleteSubscriptionGroup(groupId: string): Promise<void> {
  ...
  if (response.error) {
    if (isNotFoundError(response.error)) {
      logger.debug(`Subscription group ${groupId} was already deleted (404)`);
    } else {
      throw new ContextualError("Apple API error", {
        appleError: response.error,
      });
...
export async function deleteSubscriptionPrice(priceId: string): Promise<void> {
  const response = await api.DELETE("/v1/subscriptionPrices/{id}", {
    params: { path: { id: priceId } },
  });

  if (response.error) {
    if (isNotFoundError(response.error)) {
      logger.debug(`Subscription price ${priceId} was already deleted (404)`);
    } else {
      throw new ContextualError("Apple API error", {
        appleError: response.error,
      });
