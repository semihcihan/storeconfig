...
export async function updateAppAvailability(
  desiredTerritories: z.infer<typeof TerritoryCodeSchema>[],
  appId: string,
  currentTerritories: z.infer<typeof TerritoryCodeSchema>[]
): Promise<void> {
  logger.debug(`Target Territories:`, desiredTerritories);
  ...
  const currentTerritoriesSet = new Set(currentTerritories);
  const desiredTerritoriesSet = new Set(desiredTerritories);

  // Find territories that need to be enabled
  const territoriesToEnable = Array.from(desiredTerritoriesSet).filter(
    (territory) => !currentTerritoriesSet.has(territory)
  );

  // Find territories that need to be disabled
  const territoriesToDisable = Array.from(currentTerritoriesSet).filter(
    (territory) => !desiredTerritoriesSet.has(territory)
  );
