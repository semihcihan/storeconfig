...
export async function fetchAppAvailability(
  ...
  if (appAvailabilityResponse.error) {
    ...
    throw new ContextualError("Failed to fetch app availability", {
      appleError: appAvailabilityResponse.error,
    });
  }
  ...
  // TODO: just use getTerritoryAvailabilities function instead of this
  ...
  if (territoryAvailabilitiesResponse.error) {
    throw new ContextualError("Failed to fetch territory availabilities", {
      appleError: territoryAvailabilitiesResponse.error,
    });
  }
...
export async function createAppAvailability(
  ...
  if (response.error) {
    throw new ContextualError("Failed to create app availability", {
...
export async function updateTerritoryAvailability(
  ...
  if (response.error) {
    throw new ContextualError("Failed to update territory availability", {
      appleError: response.error,
    });
  }
}
