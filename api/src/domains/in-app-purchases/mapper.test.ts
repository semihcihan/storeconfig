import { mapLocalizations, processPriceResponse } from "./mapper";
import type { components } from "@semihcihan/app-store-connect-api-types";
import { createIncludedByIdMap } from "../../helpers/relationship-helpers";
...
jest.mock("@semihcihan/shared", () => ({
  ...
  TerritoryCodeSchema: {
    parse: jest.fn((value: string) => {
      ...
      if (validTerritories.includes(value)) {
        return value;
      }
      throw new Error("Invalid territory code");
    }),
  },
  LocaleCodeSchema: {
    safeParse: jest.fn((value: string) => {
      const validLocales = ["en-US", "en-GB"];
...
describe("In-App Purchase Mapper - mapLocalizations", () => {
  it("should prefer non-approved localization for duplicate locale", () => {
    ...
    const includedById = createIncludedByIdMap(localizations);
    const relationships = localizations.map((loc: any) => ({
      id: loc.id,
      type: "inAppPurchaseLocalizations" as const,
    }));

    const result = mapLocalizations(relationships, includedById);
    expect(result).toEqual([
      {
        locale: "en-US",
        name: "Draft Name",
