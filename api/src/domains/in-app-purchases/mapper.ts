...
export function mapLocalizations(
  ...
  localizationRels.forEach((rel) => {
    ...
    if (!locData) {
      logger.warn(`Could not find included data for localization ${rel.id}`);
      return;
    }
...
export async function fetchAndMapIAPPrices(
  priceScheduleRel:
    ...
    | undefined,
  includedById: IncludedByIdMap
): Promise<InAppPurchase["pricing"]> {
  if (!priceScheduleRel) {
    // Return undefined when there's no pricing (e.g., MISSING_METADATA state)
    logger.debug("No pricing relationship found");
    return undefined;
  }

  logger.debug(`Fetching pricing for ID: ${priceScheduleRel.id}`);

  const baseTerritoryResponse = await fetchBaseTerritory(priceScheduleRel.id);
  if (!baseTerritoryResponse.data) {
    // No base territory means no valid pricing
    throw new Error(
      `No base territory found for pricing ${priceScheduleRel.id}`
    );
  }
