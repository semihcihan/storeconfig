...
export async function mapAppPricing(
  ...
  try {
    ...
    const pricingResponse = await getAppPriceSchedule(appId);

    if (!pricingResponse?.data) {
      // No pricing found - this means pricing was not created yet
      logger.debug(`No app pricing found for app ${appId}`);
      return undefined;
    }

    const pricingId = pricingResponse.data.id;
    if (!pricingId) {
      logger.warn(`Invalid pricing ID:`, pricingResponse);
      return undefined;
    }

    // Get the base territory from the price schedule
    const baseTerritoryResponse =
      await fetchAppPriceScheduleBaseTerritory(pricingId);
    ...
    const [manualPricesResponse, automaticPricesResponse] = await Promise.all([
      fetchAppManualPrices(pricingId),
      fetchAppAutomaticPrices(pricingId, baseTerritory),
    ]);
    ...
    if (prices.length === 0) {
      logger.debug(`No prices found for app ${appId}`);
      return undefined;
    }
