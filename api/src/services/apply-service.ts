...
import { fetchInAppPurchases } from "../domains/in-app-purchases/service";
import { fetchSubscriptionGroups } from "../domains/subscriptions/service";
  ...
  switch (action.type) {
    ...
    case "UPDATE_APP_AVAILABILITY":
      await updateAppAvailability(
        action.payload.desiredTerritories,
        appId,
        action.payload.currentTerritories
      );
      break;
    ...
    case "UPDATE_APP_PRICING":
      ...
      await createAppPriceSchedule(action.payload.pricing, appId);
      break;
    ...
    case "CREATE_SUBSCRIPTION_PRICE":
      ...
      await applySubscriptionPricingWithEqualizations(
        ...
        action.payload.desiredPriceSchedule,
        action.payload.currentPriceSchedule,
        action.payload.changes
      );
      break;
...
export async function apply(
  plan: AnyAction[],
  desiredState: AppStoreModel,
  onStepCallback?: (index: number, action: AnyAction) => Promise<void>,
  dryRun: boolean = false,
  startIndex: number = 0
) {
  const appId = desiredState.appId;
  ...
  const remainingPlan = plan.slice(startIndex);
  const hasIAPActions = remainingPlan.some(
    (action) =>
  ...
  const hasSubscriptionActions = remainingPlan.some(
    (action) =>
  ...
  for (let index = startIndex; index < plan.length; index++) {
    ...
    if (onStepCallback) {
      await onStepCallback(index, action);
    }

    if (dryRun) {
      logger.debug(
        `Skipping execution of action ${action.type} due to dryRun mode`
      );
      continue;
    }
