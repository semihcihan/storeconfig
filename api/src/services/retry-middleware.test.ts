...
jest.mock("@semihcihan/shared", () => ({
  ...
  isRateLimitError: jest.fn((error: any) => {
    // Check for direct 429 status
    if (error?.status === 429 || error?.status === "429") {
      return true;
    }
    // Check for Apple API error structure
  ...
  ContextualError: class ContextualError extends Error {
  ...
  describe("Rate limit errors (429)", () => {
    ...
    it("should fail after max attempts", async () => {
      ...
      try {
        ...
        expect(String(e?.message ?? e)).toContain("GET /test/endpoint failed");
      }
