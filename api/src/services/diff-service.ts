...
import { compareNumericValues, deepEqualUnordered } from "@semihcihan/shared";
...
function diffIAPPriceSchedule(
  productId: string,
  currentSchedule: InAppPurchase["pricing"],
  desiredSchedule: InAppPurchase["pricing"]
): AnyAction[] {
  ...
  if (hasChanges) {
    actions.push({
      ...
      payload: {
        productId,
        pricing: desiredSchedule, // Always provide the complete target state
        changes: {
          addedPrices,
          updatedPrices,
          deletedTerritories,
...
export function diffInAppPurchases(
  ...
  for (const [productId, desiredIap] of desiredIapsByProductId.entries()) {
    ...
    if (!currentIap) {
      ...
      actions.push(
        ...diffIAPPriceSchedule(productId, undefined, desiredIap.pricing)
      );
    } else {
      if (currentIap.type !== desiredIap.type) {
        throw new Error(
          `The type for in-app purchase ${productId} cannot be changed. Current: ${currentIap.type}, Target: ${desiredIap.type}.`
        );
      }
...
function diffSubscriptionPrices(
  ...
  desiredPriceSchedule: z.infer<typeof PriceScheduleSchema> | undefined
): AnyAction[] {
  const actions: AnyAction[] = [];

  if (!desiredPriceSchedule) {
    return actions;
  }

  // If current price schedule is undefined, we're creating a new price schedule
  if (!currentPriceSchedule) {
    actions.push({
      ...
      payload: {
        ...
        changes: {
          addedPrices: desiredPriceSchedule.prices,
          updatedPrices: [],
          deletedTerritories: [],
        },
      },
  ...
  const current = currentPriceSchedule;

  // Check if any changes are needed
  const baseTerritoryChanging =
    current.baseTerritory !== desiredPriceSchedule.baseTerritory;

  const currentPricesByTerritory = new Map(
    current.prices.map((p: Price) => [p.territory, p])
  );
  const desiredPricesByTerritory = new Map(
    desiredPriceSchedule.prices.map((p: Price) => [p.territory, p])
  );

  // Track all price changes
  // Note: With the new equalizations approach, these have different meanings:
  // - addedPrices: Territories that are now using custom prices (different from equalized)
  // - updatedPrices: Territories that had custom prices and now have different custom prices
  // - deletedTerritories: Territories that were using custom prices and now will use equalized prices
  const addedPrices: Price[] = [];
  const updatedPrices: Price[] = [];
  const deletedTerritories: z.infer<typeof TerritoryCodeSchema>[] = [];
  ...
  for (const [territory, desiredPrice] of desiredPricesByTerritory.entries()) {
    const currentPrice = currentPricesByTerritory.get(territory);
    if (!currentPrice) {
      // Territory is new in desired state - it's now using a custom price
      addedPrices.push(desiredPrice as Price);
    } else if (
      !compareNumericValues(currentPrice.price, (desiredPrice as Price).price)
    ) {
      // Territory exists but price changed - it's still using a custom price but different
      updatedPrices.push(desiredPrice as Price);
    }
  }
  ...
  for (const [territory] of currentPricesByTerritory.entries()) {
    if (!desiredPricesByTerritory.has(territory)) {
      // Territory was using a custom price but is no longer in desired state
      // This means it will now use the equalized price
      deletedTerritories.push(territory as z.infer<typeof TerritoryCodeSchema>);
    }
  }

  // If there are any changes, generate a single comprehensive CREATE_SUBSCRIPTION_PRICE action
  const hasChanges =
    baseTerritoryChanging ||
    addedPrices.length > 0 ||
    updatedPrices.length > 0 ||
...
export function diffSubscriptions(
  ...
  for (const [productId, desiredSub] of desiredSubsByProductId.entries()) {
    ...
    if (!currentSub) {
      ...
      // Create pricing after availability is set up
      actions.push(
        ...diffSubscriptionPrices(productId, undefined, desiredSub.pricing)
      );
      ...
      if (desiredSub.subscriptionPeriod !== currentSub.subscriptionPeriod) {
        throw new Error(
          `Subscription period for subscription ${productId} cannot be changed once created. Current: ${currentSub.subscriptionPeriod}, Target: ${desiredSub.subscriptionPeriod}.`
        );
      }
...
function diffAppAvailability(
  ...
  if (
    ...
    actions.push({
      ...
      payload: {
        desiredTerritories: desiredState.availableTerritories as z.infer<
          typeof TerritoryCodeSchema
        >[],
        currentTerritories: currentState.availableTerritories as z.infer<
          typeof TerritoryCodeSchema
        >[],
      },
    });
...
function diffAppLocalizations(
  ...
  versionStringChanged: boolean = false
): AnyAction[] {
  const actions: AnyAction[] = [];
  const currentLocsByLocale = new Map(
    currentLocalizations.map((loc) => [loc.locale, loc])
  ...
  for (const [locale, desiredLoc] of desiredLocsByLocale.entries()) {
    ...
    if (!currentLoc) {
      ...
      > = {
        ...
        whatsNew: versionStringChanged ? undefined : currentLoc.whatsNew,
      };
...
export function diff(
  ...
  if (currentState.schemaVersion !== desiredState.schemaVersion) {
    throw new Error(
      `Schema version mismatch. Current: ${currentState.schemaVersion}, Target: ${desiredState.schemaVersion}. Aborting.\nRun 'npm install -g storeconfig@latest' to update StoreConfig.`
    );
  }

  if (currentState.appId !== desiredState.appId) {
    throw new Error(
      `App ID mismatch. Current: ${currentState.appId}, Target: ${desiredState.appId}. You cannot change the App ID of an existing configuration. Aborting.`
    );
  }
