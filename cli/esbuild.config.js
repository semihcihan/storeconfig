...
const workspacePathMappingPlugin = {
  name: "workspace-path-mapping",
  setup(build) {
    ...
    build.onResolve(
      { filter: /^@semihcihan\/developer-tool\/functions$/ },
      () => {
        return {
          path: path.resolve(__dirname, "../api/src/index.ts"),
          watchFiles: ["../api/src/**/*"],
        };
      }
    );
...
const cliBuildConfig = {
  ...
  packages: "bundle",
  plugins: [workspacePathMappingPlugin],
  sourcemap: true,
  minify: true,
};
