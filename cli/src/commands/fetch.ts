...
async function selectApp(apps: App[]): Promise<string> {
  ...
  logger.info(
    `Selected: ${selectedApp.name ?? selectedApp.id} (ID: ${selectedApp.id})`
  );
  ...
  handler: async (argv) => {
    ...
    if (stdout) {
      if (!appId) {
        ...
        const appStoreState = await storeConfigEngine.fetchAppStoreData(appId);
        console.log(JSON.stringify(appStoreState, null, 2));
      }
      return;
    }
    ...
    try {
      ...
      const appStoreState =
        await storeConfigEngine.fetchAppStoreData(selectedAppId);
